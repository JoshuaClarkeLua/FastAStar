...
local function doPath()
	...
	local path = AJPS.findPath(gridSize, Vector2.new(start.X,start.Z), Vector2.new(goal.X,goal.Z), nil, mainMap)
	s = os.clock() - s
	print(s)
...
local function findGoals(_goals: {Vector3})
	...
	local s = os.clock()
	local start = (origin * CFrame.new(-bSize2.X, 0, -bSize2.Z)):PointToObjectSpace(workspace.START.CFrame.Position)
	local goalsReached = AJPS.findReachable(gridSize, Vector2.new(start.X,start.Z), goals, true, mainMap)
	s = os.clock() - s
	print(s)
	print(goalsReached)
	workspace.Parts:Destroy()
